...
```
Before packaging or sharing, run `ai-context-handoff lint --dir <project>`. If it reports stale branch, commit, or timestamp metadata, re-check the current workspace before regenerating. Use `--mode analysis|fix|review|handoff` to constrain the incoming agent prompt.
